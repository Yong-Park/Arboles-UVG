...
public class HuffmanTree {
    ...
    public static byte[] compress(final String s) {
        ...
        final HuffmanTree.Node root = buildTrie(freq);
     ...
     *   build huffman tree
     **************************************************************************************************************/
    private static HuffmanTree.Node buildTrie(int[] freq) {
        final PriorityQueue<HuffmanTree.Node> pq2 = new PriorityQueue<>();

        for (char i = 0; i < R; ++i) {
            if (freq[i] > 0) {
                //pq.insert(new HuffmanTree.Node(i, freq[i], null, null));
                pq2.add(new HuffmanTree.Node(i, freq[i], null, null));
            }
        }

        if (pq2.size() == 1) {//if entire string is just one char repeated
            if (freq[0] == 0) {//empty string
                pq2.add(new HuffmanTree.Node(PARENT, 0, null, null));
            } else {
                pq2.add(new HuffmanTree.Node(EMPTY, 0, null, null));
            }
        } else
            while (pq2.size() > 1) {
                final HuffmanTree.Node left = pq2.poll();
                final HuffmanTree.Node right = pq2.poll();
                //aggregate two nodes into one by summing frequency
                final HuffmanTree.Node parent = new HuffmanTree.Node(PARENT, left.freq + right.freq, left, right);
                pq2.add(parent);
            }
     ...
     **************************************************************************************************************/
    private static void writeTrie(final HuffmanTree.Node x, final HuffmanOut binaryOut) {
     ...
     **************************************************************************************************************/
    private static void buildCode(final String[] st, final HuffmanTree.Node x, final String s) {
        if (!x.isLeaf()) {
    ...
    public static String expand(final byte[] value) {
        ...
        final HuffmanTree.Node root = readTrie(binaryIn);
        final int length = binaryIn.readInt();

        for (int i = 0; i < length; ++i) {
            HuffmanTree.Node x = root;
     ...
     *   get tree from bytes
     **************************************************************************************************************/
    private static HuffmanTree.Node readTrie(final HuffmanIn binaryIn) {
        boolean isLeaf = binaryIn.readBoolean();
        if (isLeaf) {
            char ch = binaryIn.readChar();
            return new HuffmanTree.Node(ch, UNDEFINED, null, null);
        } else {
            return new HuffmanTree.Node(PARENT, UNDEFINED, readTrie(binaryIn), readTrie(binaryIn));
        }
    }
